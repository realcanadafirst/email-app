...
const SequenceStepList = ({ stepData, setStepCallApi, setMessage }) => {
    ...
    const getStepTitle = () => stepData?.step_type == 0 ? ' Send an automatic email' : (stepData?.step_type == 1 ? ' Send an manual email' : 'Send Whatsapp message');
    ...
    const sendTestEmail = (value, issendEmail = '') => {
        ...
        if (!value) {
            ...
            setSendMessageStatus(false); // onClick={() => sendTestEmail(true, 'message')}
        }
        ...
        <>
            {
                stepData ? <div className="col-span-12 mb-9 rounded-sm border border-stroke bg-white drop-shadow-1 dark:border-strokedark dark:bg-boxdark xl:col-span-4">
                    <div className="flex justify-between px-7.5 py-3">
                        ...
                        {
                            stepData.step_type !== '2' ? <button className="text-sm text-primary" onClick={() => { sendTestEmail(true, 'sendEmail'); }}> Send Test email</button>
                                : null
                        }
                    </div>
                    {
                        stepData.step_type !== '2' ? <div className="px-7.5 py-3 hover:bg-gray-3 dark:hover:bg-meta-4 cursor-pointer" onClick={() => sendTestEmail(true)}>
                            ...
                            : <div className="px-7.5 py-3 hover:bg-gray-3 dark:hover:bg-meta-4 cursor-pointer" > 
                                <div className="flex flex-1 items-center justify-between">
                                    <p className="text-sm"> Message :<span className="text-sm"> {stepData.whatsapp_message}</span></p>
                                </div>
                            </div>
                    }
            ...
            <EmailAppModal
                ...
                {
                    sendMessageStatus ? <AddTemplateForm handleChange={handleChange} formData={formData} dataChange={dataChange} sendTestEmailStatus={sendTestEmailStatus} readonly={sendMessageStatus} /> : <AddTemplateForm handleChange={handleChange} formData={formData} dataChange={dataChange} sendTestEmailStatus={sendTestEmailStatus} readonly={sendTestEmailStatus} />
                }
            </EmailAppModal>
        </>
    );
