...
async function handlePostRequest(req, res) {
    try {
        ...
        let query = 'INSERT INTO emails (mail_from, receivers, subject, template) VALUES (?, ?, ?, ?)';
        const values = [mail_from, receivers, subject, template];
        if (id) {
            query = `UPDATE emails SET mail_from = ?, receivers = ?, subject = ?, template = ? WHERE id = ?`;
            values.push(id);
        }
