    ...
    try {
        ...
        try {
            ...
            if (emailData && emailData.length) {
                ...
                const [appData] = await connection.execute(`SELECT * FROM settings WHERE user_hash = '${emailDataResult['user_hash']}'`);
                if (appData) {
                    const appDataResult = appData[0];
                    const [prospects] = await connection.execute(`SELECT * FROM email_prospects WHERE email_id = ${emailDataResult['id']} AND email_sent = '2'`);
                    if (prospects) {
